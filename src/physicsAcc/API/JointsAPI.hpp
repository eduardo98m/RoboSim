...
struct PrismaticJointParams {
  ...
  vec3 r_1 = (vec3){0.0, 0.0, 0.0};         // Default being the COM of the body
  vec3 r_2 = (vec3){0.0, 0.0, 0.0};         // Default being the COM of the body
 ...
 *
 * This function creates a primsatic joint, which allows straight movement on a
 * single axis, between two specified bodies. It involves creating multiple
 * constraints in the ConstraintCollection to simulate the joint's behavior.
 *
 * @param jc The JointCollection to store the joint data.
 * @param cc The ConstraintCollection to store the constraints associated with
 * the joint.
 * @param params The parameters defining the primsatic joint.
 * @return The ID of the created joint.
 */
size_t create_prismatic_joint(JointCollection &jc, ConstraintCollection &cc,
                              PrismaticJointParams params) {

  // Check the correct order
  // Alignment
  size_t c_1 = create_constraint(
      cc, ConstraintsParams{.body_1 = params.body_1,
 ...
 *
 * This function creates a revolute joint, which allows rotation around a single
 * axis, between two specified bodies. It involves creating multiple constraints
 * in the ConstraintCollection to simulate the joint's behavior.
 *
 * @param jc The JointCollection to store the joint data.
 * @param cc The ConstraintCollection to store the constraints associated with
 * the joint.
 ...
 * @return The ID of the created joint.
 */
size_t create_revolute_joint(JointCollection &jc, ConstraintCollection &cc,
 ...
 * @return The ID of the created joint.
 */
size_t create_fixed_joint(JointCollection &jc, ConstraintCollection &cc,
                          FixedJointParams params) {
  // Alignment constraint
  size_t c_1 = create_constraint(
      cc, ConstraintsParams{.body_1 = params.body_1,
                            .body_2 = params.body_2,
  ...
  jc.actuation_type.push_back(
      JointActuationType::FREE); // Fixed joints don't have actuation
