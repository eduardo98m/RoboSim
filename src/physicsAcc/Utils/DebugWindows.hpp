...
struct DebugWindow {
  ...
  std::string cached_data; // ¡Nuevo! Para almacenar la última información
                           // cuando está pausada
};
...
public:
  ...
  void render_debug_uis(World &world) {
    ...
    ImGui::Text("Global Pause Status: %s",
                global_pause_active ? "PAUSED" : "RUNNING");
    ...
    for (auto &window : debug_windows) {
      if (window.is_open) {
        render_single_debug_window(
            window, world); // Pasa world a la función de renderizado
      }
    }
    // Clean the erased windows
    debug_windows.erase(
...
private:
  ...
  void add_debug_window(const std::string &name) {
    ...
    debug_windows.push_back(
        {name, DebugEntityType::NONE, 0, true, global_pause_active, ""});
  }

  /**
   * @brief Caches the data of a debug window (Thjis si for the pause feature)
   */
  void cache_window_data(DebugWindow &debug_window, World &world) {
    ...
    case DebugEntityType::BODY:
      if (debug_window.entity_index < world.bodies.n_bodies) {
        ss << get_generic_info(world.bodies, debug_window.entity_index,
                               "BODY DATA");
        valid_index = true;
      }
      break;
    case DebugEntityType::CONSTRAINT:
      if (debug_window.entity_index < world.constraints.n_constraints) {
        ss << get_generic_info(world.constraints, debug_window.entity_index,
                               "CONSTRAINT DATA");
        valid_index = true;
      }
      break;
    case DebugEntityType::JOINT:
      if (debug_window.entity_index < world.joints.n_joints) {
        ss << get_generic_info(world.joints, debug_window.entity_index,
                               "JOINT DATA");
        valid_index = true;
      }
      break;
  ...
  void render_single_debug_window(DebugWindow &debug_window, World &world) {
    ...
    if (ImGui::Checkbox("Pause This Window", &debug_window.is_paused)) {
      ...
      // Si se despausa, asegura que la pausa global también esté desactivada
      // para esta ventana (aunque la pausa global puede seguir activa para
      // otras)
    ...
    if (debug_window.is_paused) {
      ...
      ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.0f, 1.0f),
                         "Displaying Cached Data:"); // Mensaje en verde
      ...
      if (ImGui::Combo("Entity Type", &current_type_idx, entity_types,
                       IM_ARRAYSIZE(entity_types))) {
        debug_window.selected_type =
            static_cast<DebugEntityType>(current_type_idx);
      }
      ...
      case DebugEntityType::BODY:
        if (debug_window.entity_index < world.bodies.n_bodies) {
          ...
          ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f),
                             "Invalid Body Index! Max: %ld",
                             world.bodies.n_bodies - 1);
        }
        break;
      case DebugEntityType::CONSTRAINT:
        if (debug_window.entity_index < world.constraints.n_constraints) {
          ...
          ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f),
                             "Invalid Constraint Index! Max: %ld",
                             world.constraints.n_constraints - 1);
        }
        break;
      case DebugEntityType::JOINT:
        if (debug_window.entity_index < world.joints.n_joints) {
          ...
          ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f),
                             "Invalid Joint Index! Max: %ld",
                             world.joints.n_joints - 1);
        }
        break;
      case DebugEntityType::NONE:
      default:
