...
Texture::Texture(const std::string &base_name)
{
    this->base_name = base_name;
}


std::string Texture::get_short_name_of_texture_type(TextureType texture_type)
{
...
std::string Texture::get_long_name_of_texture_type(TextureType texture_type)
{
    if (texture_type == TexAlbedo)
    {
...
{
    std::string output_name = this->base_name;
...
// render the mesh
void Mesh::draw(Shader *shader, Material *draw_material, bool is_selected, bool disable_depth_test, bool render_only_ambient, bool render_one_color)
{
    ...
    shader->set_int("material_format", draw_material->format);
    ...
    for (int type = TexAlbedo; type < TexLast; type++)
    {
        ...
        if (draw_material->textures.find(texture_type) != draw_material->textures.end())
        {
            ...
            shader->set_int(str_texture_type, OFFSET_TEXTURES + num_active_textures);
            shader->set_int("has_" + str_texture_type, true);
            glBindTexture(GL_TEXTURE_2D, texture->id);
            num_active_textures++;
        }
        else
        {
            shader->set_int("has_" + str_texture_type, false);
        }
    }

    shader->set_int("render_only_ambient", render_only_ambient);
    shader->set_int("render_one_color", render_one_color);
    shader->set_int("paint_selected_texture", is_selected);
...
bool Mesh::intersected_ray(const glm::vec3 &orig, const glm::vec3 &dir, float &t)
{
